...
- **2026-04-19: 8×28 = 224 active display area.** Fortinbra clarified GIF content will always be 28 wide × 8 tall = 224 LEDs. The physical panel is still 8×32 = 256 LEDs. The last 4 columns (28–31) are intentionally dark. Content is left-aligned at column 0, not centered. This is the permanent content size, not a workaround. The firmware's existing clear-then-blit path already handles this correctly. The key stale artifact is `collect-proof.ps1` line 92 which still searches for "154 pixels" strings that no longer exist in the ELF.
- When user says "rows" in context of this serpentine panel, they may mean columns (each column is a physical row of 8 LEDs in the chain). Always verify with the numeric LED count.
